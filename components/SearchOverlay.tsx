...
const SearchOverlay = ({ isOpen, onClose, data = [] }: SearchOverlayProps) => {
        ...
        <AnimatePresence>
            {isOpen && (
                <motion.div
                    ...
                    animate={{ opacity: 1 }}
                    exit={{ opacity: 0 }}
                    className="fixed inset-0 z-[200] bg-black/95 backdrop-blur-xl flex flex-col text-white"
                >
                    {/* Header */}
                        ...
                        {query.trim() === '' ? (
                            ...
                            <div className="space-y-4">
                                ...
                                {filteredResults.map((result, index) => (
                                    ...
                                    >
                                        <Link
                                            href={result.title ? `/nominees?categoryId=${result.id}` : `/profile/${result.id}`}
                                            onClick={onClose}
